    ...
    private void handlePotentialDeadlock(Lock lock, long transNum, LockType lockType) {
        ...
        if ((lockType.equals(LockType.EXCLUSIVE)) || (lock.getType().equals(LockType.EXCLUSIVE))) {
            for (Long owner : lock.getOwners()) {
                waitsForGraph.addNode(owner);
                if ((waitsForGraph.edgeCausesCycle(transNum, owner)) && (transNum != owner)) {
                    throw new DeadlockException("Deadlock Exception Error when adding edge between " + transNum + " and " + owner + "!!!!!");
                } else {
                    if (transNum != owner) {
                        waitsForGraph.addEdge(transNum, owner);
                    }
                }
            }
        }
